...
class Conti_Dataset(Dataset):
    ...
    def __init__(self, train_loader, task_id):
        ...
        self.length = len(train_loader[str(task_id)][0].dataset)
        
        for task in range(task_id+1):
            self.data.extend(torch.unbind(torch.tensor(train_loader[str(task)][0].dataset["input_ids"]), dim=0))
            self.labels.extend([task_id] * len(train_loader[str(task)][0].dataset))
...
def get_task_feature_vector(train_loader, task_id, lm, output_dir, margin=2, epochs=5):
    ...
    for task in range(task_id+1):
        ...
        ### 전에 학습한 Task의 Task feature vector (768차원)에서 100개의 데이터를 증강하여 학습, 
        ### subset의 dataset: task feature vector에서 100개의 data (256개의 token id)를 랜덤으로 뽑아서 학습
