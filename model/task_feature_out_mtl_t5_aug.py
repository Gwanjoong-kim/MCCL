...
class Conti_Dataset(Dataset):
    ...
    def __init__(self, train_loader, task, output_dir):
        ...
        dataset_dict = {"0": {'0': "dbpedia", '1': "yahoo", '2': "amazon", '3': "yelp", '4': "agnews"},
                        "1": {'0': "yahoo", '1': "amazon", '2': "yelp", '3': "agnews", '4': "dbpedia"},
                        "2": {'0': "dbpedia", '1': "yahoo", '2': "agnews", '3': "amazon", '4': "yelp"},
                        "3": {'0': "yelp", '1': "agnews", '2': "dbpedia", '3': "amazon", '4': "yahoo"}}
        std = 5
        # pdb.set_trace()
        self.data.extend(torch.tensor(train_loader[dataset_dict[output_dir[36]][str(task)]][0].dataset["source_ids"])[:100])
        self.labels.extend([task] * 100)
                
        for t in range(task):
            mean_tensor = torch.tensor(mean_of_input_query[str(t)], dtype=torch.float32)
            sampled_ids = (mean_tensor + torch.randn(100, 256) * std).int()
...
class SimpleEmbeddingNet(nn.Module):
    ...
    def forward(self, x):
        # (B, 512, 1024) -> (B, 1024)
        # pdb.set_trace()
        x = self.lm.encoder.embed_tokens(x.to(self.device))
...
def train_epoch(model, dataloader, margin=2.0):
    ...
    for x, y in dataloader:
        ...
        # Contrastive Loss
        loss = contrastive_loss_multi_class(outputs, y, device=model.device, margin=margin)
