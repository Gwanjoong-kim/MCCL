...
class PrefixContinualMTLEncoder(nn.Module):
    ...
    def forward(self, batch_size, task_id, embed_prototypes, train=True, final=False, prompt_select_mode='local_compose', task_id_list=None, id_pred_on=False): # here local_compose means local concatenation; global_compose means global concatenation (for Progressive Prompts)
...
class PrefixQKVEncoder(nn.Module):
    ...
    def forward(self, x_query, task_id, embed_prototypes, train=True, final=False, prompt_select_mode='local_compose', task_id_list=None, id_pred_on=False):
        if train and not final: # training
            self.steps[task_id] += 1
        elif not train and not final: # validation
            self.steps_val[task_id] += 1
        ...
        # condition = torch.tensor((task_id >= self.task_limit), dtype=torch.bool)
        # real_ids = torch.where(
        #     condition,
        #     task_id - self.task_limit,
        #     task_id
        #     )
        
        task_list = list(range(self.num_tasks))
        task_list[self.task_limit:] = task_list[:len(task_list)-self.task_limit]
        real_ids = torch.tensor(task_list)
        
        K = self.keys[real_ids]
        P = self.prompts[real_ids]
        ...
        # This mode
            ...
            # pdb.set_trace()
            # print("compose prompt 하기 전 embed_prototypes: ", embed_prototypes, flush = True)
            K, P, cos_sim = self._compose_prompts(x_query, K, P, task_id, embed_prototypes, train, prompt_select_mode, A, task_id_list, id_pred_on)
            # print("compose 함")
        ...
        # This mode    
            ...
            # This mode
                ...
                # P_ = “n 차원을 따라 P를 w로 가중합한 결과"
                P_ = torch.einsum('bn,nkld->bkld', w, P[:w.shape[-1]]) #bxNlx2lxd ## Error
                # print("w: ",w, flush = True)
                with open(f'{self.config.output_dir}/w.pkl', 'wb') as f:
                    pickle.dump(w, f)
    ...
    def _initialize_keys_prompts(self):
        ...
        ########## 애초에 key와 prompt를 task의 개수만큼 만들어 놓음 ##########
        ...
        # prompts[task_limit:] = prompts[:self.num_tasks-task_limit]
    ...
    def _compose_prompts(self, query, K, P, task_id, embed_prototypes, train, prompt_select_mode, A=None, task_id_list=None, id_pred_on=False):
        ...
        if (self.config.epi_for_composition or self.config.embed_prototypes_for_composition) and self.config.compose_prompts:
            if self.config.epi_for_composition:
                ...
                if not train and id_pred_on and task_id_list is not None:
                    task_id_max = max(task_id_list.cpu()).item()
                    cos_sim = self._get_task_embed_pred(query, task_id_max, embed_prototypes, train)
                else:   
                    # This mode
                    cos_sim = self._get_task_embed_pred(query, task_id, embed_prototypes, train)
                    # print("cos_sim: ", cos_sim, flush = True)
        else:         
    ...
    def _get_task_embed_pred(self, query, task_id, embed_prototypes, train):
        device = embed_prototypes[0].device  # 첫 번째 텐서의 디바이스 확인
        embed_prototypes = [tensor.to(device) for tensor in embed_prototypes[:task_id+1]]  
        embed_prototypes = torch.stack(embed_prototypes[:task_id+1])
        # print("query shape: ", query.shape, flush = True)
        # print("embed_prototypes shape: ", embed_prototypes[:task_id+1].shape, flush = True)
        # cos_sim = torch.einsum('bj,kj->bk', query, embed_prototypes.cuda())
        # cos_sim.softmax(dim=-1인지 dim=0인지에 따라 weight distribution per task가 달라짐)
        cos_sim = torch.nn.functional.cosine_similarity(query.unsqueeze(1), embed_prototypes.cuda().unsqueeze(0), dim=2)
        # pdb.set_trace()
        # print("pred_task_id: ", cos_sim.argmax(dim=1), flush = True)
    ...
    def process_task_count(self):
        self.task_count += 1
        ### 여기서 task_feature_out의 get_task_feature_out을 호출해서 task_feature_out을 얻어야 함 contrastive learning일 떄, 
        ### 한 task의 전부 embed를 얻은 후 contrastive learn
        ...
        if self.task_count > 1 and self.config.composed_prompt_for_usage:
            ...
            # print("w: ", w.shape, flush = True)
            # pdb.set_trace()
            P_i = torch.einsum('n,nkld->kld', w, P)
            ### input0과 각 task의 embed_prototypes간 cosine-smilarity 차이 계산
            with torch.no_grad():
