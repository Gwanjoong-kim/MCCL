...
class RobertaPrefixContinualForSequenceClassification(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        if self.config.compose_prompts or self.config.task_identify_epi:
            ...
            if self.save_sample[task_id] == False:
                pickle.dump(x_query, open(f"{self.config.output_dir}/{task_id}_x_query.pkl", "wb"))
        ...
        # print("get_prompt 실행 전 embed_prototypes: ", self.embed_prototypes)
    ...
    def process_query(self, embed, attention_mask, train, task_id, labels=None):
        ...
        pickle.dump(self.embed_prototypes, open(f"{self.config.output_dir}/til_{task_id}_embed_prototypes.pkl", "wb"))    
        return x_query
    ...
    def _get_pred_ids(self, x_query, task_id, final, log_id='cil'):
        if self.config.classifier_match_embed:
            embed_prototypes = torch.stack(self.embed_prototypes).to(x_query.device)
