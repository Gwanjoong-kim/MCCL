...
from model.sequence_classification_bert_with_contrastive_learning import BertPrefixContinualForSequenceClassification_contrastive_learning
from model.sequence_classification_roberta import RobertaPrefixContinualForSequenceClassification
from model.sequence_classification_roberta_with_contrastive_learning import RobertaPrefixContinualForSequenceClassification_contrastive_learning
...
PREFIX_MODELS = {
    "bert": {
        ...
        # "contrastive": {TaskType.SEQUENCE_CLASSIFICATION: BertPrefixContinualForSequenceClassification}
    },
    # "roberta": {
...
MTL_MODELS = {
    "prefix":{
        "bert": {
            ...
            "contrastive": {
                TaskType.SEQUENCE_CLASSIFICATION: BertPrefixContinualForSequenceClassification_contrastive_learning
            }
        },
        # "roberta": {
        ...
        "xlm-roberta": {
            ...
            "contrastive": {
                TaskType.SEQUENCE_CLASSIFICATION: RobertaPrefixContinualForSequenceClassification_contrastive_learning
                }
        },
    },
}
...
def get_model(model_args, task_type: TaskType, config: AutoConfig, fix_bert: bool = False, language=None, seed=None, mtl=False, mtl5=False, max_seq_len=None, output_dir=None, label_list=None, contrastive_feature=False, task_limit=None):
    ...
    if model_args.prefix:
        ...
        if mtl:
            if contrastive_feature:
                model_class = MTL_MODELS['prefix'][config.model_type]["contrastive"][task_type]
                                
            else:
                model_class = MTL_MODELS['prefix'][config.model_type]["naive"][task_type]
...
def get_model_bert(model_args, task_type: TaskType, config: AutoConfig, fix_bert: bool = False, language=None, seed=None, mtl=False, mtl5=False, max_seq_len=None, output_dir=None, label_list=None, contrastive_feature=False):
    for arg in dir(model_args):
        if not arg.startswith("__") and not callable(getattr(model_args, arg)):
            setattr(config, arg, getattr(model_args, arg))
    setattr(config, 'max_seq_length', max_seq_len)
