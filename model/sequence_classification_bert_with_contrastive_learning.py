...
import model.task_feature_out_bert as task_feature_out
import pickle
import pdb


class BertPrefixContinualForSequenceClassification_contrastive_learning(BertPreTrainedModel):
    ...
    def _get_pred_ids(self, x_query, task_id, final, train, log_id='cil'):
        if self.config.classifier_match_embed:
            if train:
                embed_prototypes = torch.tensor(self.embed_prototypes)
